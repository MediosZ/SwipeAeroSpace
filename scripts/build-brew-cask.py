...
def main():
    ...
    parser.add_argument(
        "--build-version", type=str, required=True, help="Build version"
    )
    ...
    # Calculate SHA256
    sha = (
        subprocess.check_output(["shasum", "-a", "256", zip_file])
        .decode("utf-8")
        .split()[0]
    )
  ...
  postflight_steps do
    run "/usr/bin/xattr", args: ["-d", "com.apple.quarantine", "{{{{appdir}}}}/{app_bundle_dir_name}"], must_succeed: false
  end
